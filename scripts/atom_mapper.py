...
def prediction2map(rxn, prediction, fix_product_mapping = False, return_template = True, neighbor_weight = 10):
    ...
    if return_template:
        try:
            return {'mapper': mapper, 'mapped_rxn': mapped_rxn, 'template': extract_from_reaction(mapped_rxn)}
        except:
            return {'mapper': mapper, 'mapped_rxn': mapped_rxn, 'template': None}
        return 
    else:
        return {'mapper': mapper, 'mapped_rxn': mapped_rxn}
...
#         # in case of multiple reagents
    ...
    def plot_prediction(self, plot_raw = False):
        ...
        if plot_raw:
            ...
            plt.imshow(self.masked_predictions, cmap = 'coolwarm')
        plt.colorbar()
